...
export function mapRegistrationError(err: unknown): import("./registration").RegistrationFieldErrors {
  if (err instanceof ClientApiError) {
    switch (err.code) {
      case "USERNAME_TAKEN":
...
export function mapLoginError(err: unknown): import("./registration").LoginFieldErrors {
  if (err instanceof ClientApiError) {
    switch (err.code) {
      case "INVALID_CREDENTIALS":
